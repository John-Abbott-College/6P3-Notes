...
. Unfortunately, these methods (using `apt`) depend on unmaintained packages that cause issues.
:::
...
You can later add these packages to a `requirements.txt` or `pyproject.toml` file for projects you need these tools regularly.
...
Then, need to delete your existing virtual environment and recreate it.
:::
