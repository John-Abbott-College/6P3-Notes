---
title: Course Info
description: |
  Course description outlining goals, expectations, resources, and other
  important information.
date: 2025-01-20
---

## Logistics

- **Room, times, etc.:** see [Course Outline]({{< var course.outline >}})
- **Office:** Penfield 311
- **Office hours:** Mon/Fri 11:00 - 12:30pm or so
- **Email:** michael DOT haaf AT johnabbott DOT qc DOT ca
- **Course webpage:** This website for all content, [Moodle]({{< var site.moodle >}}) for assignment/project/quiz submissions.

:::{.column-margin}
![Forgive me the smarmy comic, this is usually true...](assets/itsInTheSyllabus.png)
:::

### Communication

- Teams - For communicating (fastest, most reliable that I will check it
  same-day)
- MIO - For communicating (non-time-critical)
- Email - For communicating (time-critical). Will aim for <24hr response.
- [Moodle]({{< var site.moodle >}}) - For receiving & submitting exercises/assignments/project
  and getting marks

### Assessment

- 45% Assignments and Labs
- 20% Test (first week of April)
- 35% Project Milestones

## Course material

There are no required textbooks for this course -- this website will contain all
of the content that you need to complete course deliverables. Additionally, each
lecture will contain references to additional resources for exploring each topic
in further detail beyond the scope of the course.
